...
func roundtableStageGuidance(stage string) string {
	...
	case "frozen_diff":
		return "This frozen diff is the implemented deliverable. Required edits that are absent, or edits that substitute a different goal or deliverable, are drift and must fail closed. A patch is not the complete repository: unchanged definitions are normally absent from it. Never infer that a symbol, route, test, or behavior is absent merely because it is not shown in the patch; omit that claim and state uncertainty only in a non-blocking suggestion. A patch artifact does not normally contain command output or version-control metadata. Their absence from the patch is not evidence that tests, requested commands, or commits were omitted, so never create a blocking finding solely because the patch does not embed those logs or metadata. Use only the supplied context and do not inspect the worktree or call tools."
	}
